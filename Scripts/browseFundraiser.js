  ...
  .addEventListener("click", function () {
    ...
    let res = browseFundraiserDataDefault.filter(function (el) {
      return (
        ...
        el.by.toLowerCase().includes(input.value.toLowerCase())
      );
    });
    disp(res);
  });
...
function disp(res) {
  ...
  res.forEach(function (el) {
    ...
    divParent.append(img, div,button);
    document.querySelector("#innerData").append(divParent);

    button.addEventListener("click", function() {
      if (el.DaysLeft != "Fundraise Ended"){
      localStorage.setItem("DonateData",JSON.stringify(el));
      location.href ="./donate.html"
      }else{
        alert("Fundraise has ended for this event")
      }
    })
